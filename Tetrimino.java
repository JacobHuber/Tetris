...
public class Tetrimino { 
	 ...
	 * @param color
	 */
	public Tetromino(TetriminoShape shape, Color color, int rotationDegrees) {
		Block[] arrBlock = new Block[BLOCK];
		this.blocks = arrBlock;
		this.shape = shape;		
		this.color = color;
		this.rotation = rotationDegrees/90;
		startingBlocks(shape, arrBlock);
	}
...
/* End of getters and setters */
    ...
    private void StartingBlocks(TetriminoShape minoShape, Block[] bloc) {
        ...
        for (int i = 0; i < minoShape.getTetrisLocations().length; i++) {
            bloc[i] = new Block(minoShape.getColor(),
            		minoShape.getTetrisLocations()[i]);
        }
    }
